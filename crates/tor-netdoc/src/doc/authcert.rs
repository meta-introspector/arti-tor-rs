...
pub mod tmp {
    ...
    use crate::{
        NormalItemArgument,
        parse2::{
            ErrorProblem, ItemObjectParseable, NdaSystemTimeDeprecatedSyntax, SignatureHashInputs,
            VerifyFailed, check_validity_time_tolerance,
        },
        types,
    };
    ...
    pub struct DirAuthKeyCert {
        ...
        pub kp_auth_id_rsa_sha1: (types::Fingerprint,),
        ...
        pub kp_auth_id_rsa: rsa::PublicKey,
        ...
        pub kp_auth_sign_rsa: rsa::PublicKey,
        ...
        pub dir_key_crosscert: NdiDirKeyCrossCert,
    }

    /// Signatures for [`DirAuthKeyCert`].
    ///
    /// Signed by [`DirAuthKeyCert::kp_auth_id_rsa`] in order to prove ownership.
    ...
    pub struct DirAuthKeyCertSignatures {
        ...
        pub dir_key_certification: NdiDirKeyCertification,
    }
    ...
    #[non_exhaustive]
    pub struct NdiDirKeyCertification {
    ...
    /// is reserved for full-body signatures, such as [`NdiDirKeyCertification`].
    ...
    /// identity key alongside the [`NdiDirKeyCrossCert`] must perform proper
    ...
    #[deftly(netdoc(no_extra_args))]
    #[non_exhaustive]
    pub struct NdiDirKeyCrossCert {
        /// The bytes of the signature (base64-decoded).
        #[deftly(netdoc(object))]
        pub signature: NdiDirKeyCrossCertObject,
    }
    ...
    #[non_exhaustive]
    pub struct NdiDirKeyCrossCertObject(pub Vec<u8>);
    ...
    impl DirAuthKeyCertSigned {
        ...
        pub fn verify_selfcert(
            ...
            // (1) Check whether this comes from a valid authority in `v3idents`.
            if !v3idents.contains(&body.kp_auth_id_rsa_sha1.0) {
            ...
            // (3) Check whether the fingerprint and long-term identity key match.
            if body.kp_auth_id_rsa.to_rsa_identity() != *body.kp_auth_id_rsa_sha1.0 {
                return Err(VerifyFailed::Inconsistent);
            }

            // (4) Check the cross-certificate (proof-of-ownership of signing key).
            body.kp_auth_sign_rsa.verify(
                body.kp_auth_id_rsa_sha1.0.as_bytes(),
                &body.dir_key_crosscert.signature,
            )?;

            // (5) Check the outer certificate (proof-of-ownership of identity key).
            body.kp_auth_id_rsa.verify(
    ...
    impl Deref for NdiDirKeyCrossCertObject {
        type Target = Vec<u8>;
    ...
    impl ItemObjectParseable for NdiDirKeyCrossCertObject {
...
mod test {
    ...
    mod tmp {
        ...
        use crate::{
            parse2::{self, ErrorProblem, ParseError, VerifyFailed},
        ...
        /// Tests whether a [`NdiDirKeyCrossCert`] can be parsed properly.
        #[test]
        fn dir_auth_cross_cert() {
            #[derive(Debug, Clone, PartialEq, Eq, Deftly)]
            #[derive_deftly(NetdocParseable)]
            struct Dummy {
                dir_key_crosscert: NdiDirKeyCrossCert,
            }
            ...
            let res = parse2::parse_netdoc::<Dummy>(&cert, "").unwrap();
            assert_eq!(
                res,
                Dummy {
                    dir_key_crosscert: NdiDirKeyCrossCert {
                        signature: NdiDirKeyCrossCertObject(decoded.clone())
                    }
                }
            );
            ...
            let res = parse2::parse_netdoc::<Dummy>(&cert, "");
            match res {
                Err(ParseError {
        ...
        fn dir_auth_key_cert_signatures() {
            ...
            let res = parse2::parse_netdoc::<DirAuthKeyCertSignatures>(&cert, "").unwrap();
            assert_eq!(
                res,
                DirAuthKeyCertSignatures {
                    dir_key_certification: NdiDirKeyCertification {
            ...
            let res = parse2::parse_netdoc::<DirAuthKeyCertSignatures>(&cert, "");
            match res {
                Err(ParseError {
                    problem: ErrorProblem::ObjectIncorrectLabel,
                    doctype: "",
        ...
        fn dir_auth_cert() {
            ...
            let res = parse2::parse_netdoc::<DirAuthKeyCert>(&input, "").unwrap();
            assert_eq!(
                res,
                DirAuthKeyCert {
                    dir_key_certificate_version: (NdaDirKeyCertificateVersion::V3,),
                    kp_auth_id_rsa_sha1: (types::Fingerprint(
                    ...
                    kp_auth_id_rsa: rsa::PublicKey::from_der(&to_der(include_str!(
                        "../../testdata2/authcert-longclaw-id-rsa"
                    )))
                    .unwrap(),
                    kp_auth_sign_rsa: rsa::PublicKey::from_der(&to_der(include_str!(
                        "../../testdata2/authcert-longclaw-sign-rsa"
                    )))
                    .unwrap(),
                    dir_key_crosscert: NdiDirKeyCrossCert {
                        signature: NdiDirKeyCrossCertObject(
        ...
        fn dir_auth_signature() {
            let res = parse2::parse_netdoc::<DirAuthKeyCertSigned>(
