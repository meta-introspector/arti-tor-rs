...
pub struct PathConfig {
    ...
    #[deftly(TorConfig(default = "ipv4_prefix_default()"))]
    ...
    #[deftly(TorConfig(default = "ipv6_prefix_default()"))]
    ipv6_subnet_family_prefix: u8,

    /// A set of ports that need to be sent over Stable circuits.
    #[deftly(TorConfig(sub_builder))]
    #[deftly(TorConfig(setter(skip)))] // Provided by define_list_builder_accessors
...
pub struct PreemptiveCircuitConfig {
    ...
    #[deftly(TorConfig(default = "default_preemptive_threshold()"))]
    ...
    #[deftly(TorConfig(default = "default_preemptive_duration()"))]
    pub(crate) prediction_lifetime: Duration,

    /// How many available circuits should we try to have, at minimum, for each
    /// predicted exit port?
    #[deftly(TorConfig(default = "default_preemptive_min_exit_circs_for_port()"))]
...
pub struct CircuitTiming {
    ...
    #[deftly(TorConfig(default = "default_max_dirtiness()"))]
    #[getter(skip)]
    ...
    #[deftly(TorConfig(default = "default_disused_timeout()"))]
    #[getter(skip)]
    ...
    // TODO: Impose a maximum or minimum?
    #[deftly(TorConfig(default = "default_request_timeout()"))]
    #[getter(skip)]
    ...
    /// this many attempts.
    // TODO: Impose a maximum or minimum?
    #[deftly(TorConfig(default = "default_request_max_retries()"))]
    #[getter(skip)]
    ...
    /// request.
    #[deftly(TorConfig(default = "default_request_loyalty()"))]
    #[getter(skip)]
    ...
    //
    // NOTE: we could use #[deftly(TorConfig(cfg = ))] rather than #[cfg] here, if we wanted to give
    // a warning when the user provided a setting for this value while hs-client was compiled out.
    #[cfg(feature = "hs-client")]
    #[deftly(TorConfig(default = "default_hs_max_attempts()"))]
    #[getter(as_copy)]
